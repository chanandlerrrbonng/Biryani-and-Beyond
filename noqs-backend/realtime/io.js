...
function initRealtime(httpServer) {
  ...
  io.on('connection', (socket) => {
    console.log(`[socket] merchant connected: ${socket.id}`);
    inboxHandlers.register(socket, io);
    socket.on('disconnect', () => console.log(`[socket] merchant disconnected: ${socket.id}`));
  });
...
function emitConversationUpdate(payload) {
  if (io) io.emit('conversation:update', payload);
}

function emitHumanInbound(payload) {
  if (io) io.emit('conversation:human', payload);
}

module.exports = { initRealtime, getIO, emitConversationUpdate, emitHumanInbound };
